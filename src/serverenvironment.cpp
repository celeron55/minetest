		...
		if (!found) {
			...
			if (c_id == CONTENT_IGNORE) {
				...
				warningstream << "Could not internalize node name \"" << nodeTrigger
						<< "\" while loading LBM \"" << lbm_def->name <<
						"\"." << std::endl;
				continue;
			}
		...
		std::set<v3s16> &blocks_added)
{
	...
	*/
	...
	for (const PlayerSAO *playersao : active_players) {
		...
		s16 player_ao_range = std::min(active_object_range,
				playersao->getWantedRange());
		...
		*new_player = true;
		...
		if (objectpos_over_limit(playersao->getBasePosition())) {
			actionstream << "Respawn position for player \""
					<< player->getName() << "\" outside limits, resetting" <<
					std::endl;
			playersao->setBasePosition(m_server->findSpawnPos());
		}
	}
...
public:
			...
			int &blocks_cached)
	{
		...
		u32 active_object_count = this->countObjects(block, map,
		...
		for (p0.Z = 0; p0.Z<MAP_BLOCKSIZE; p0.Z++)
		{
			...
			neighbor_found:
				...
				if (m_env->m_added_objects > 0) {
					active_object_count = countObjects(block, map,
							active_object_count_wider);
					m_env->m_added_objects = 0;
				}
			}
...
void ServerEnvironment::step(float dtime)
{
	...
	*/
	if (m_active_blocks_management_interval.step(dtime,
	...
	*/
	if (m_active_blocks_nodemetadata_interval.step(dtime,
		...
		std::queue<u16> &added_objects)
{
	...
	m_ao_manager.getAddedActiveObjectsAroundPos(
			playersao->getBasePosition(), radius_f,
...
{
	ScopeProfiler sp(g_profiler, "ServerEnvironment::removeRemovedObjects()",
			SPT_AVG);
	...
	auto clear_cb = [this](ServerActiveObject *obj, u16 id){
		...
		*/
		if (obj->m_pending_removal)
			deleteStaticFromBlock(obj, id, MOD_REASON_REMOVE_OBJECTS_REMOVE,
			false);
...
void ServerEnvironment::deactivateFarObjects(bool _force_delete)
{
	auto cb_deactivate = [this, _force_delete](ServerActiveObject *obj, u16 id){
		...
		m_active_blocks.contains(blockpos_o)) {
			...
			deleteStaticFromBlock(obj, id, MOD_REASON_STATIC_DATA_REMOVED,
			false);
		...
		*/
		if (obj->isStaticAllowed()) {
			...
			*/
			...
			u32 reason =
					shall_be_written ? MOD_REASON_STATIC_DATA_CHANGED :
			MOD_REASON_UNKNOWN;
		...
		ServerActiveObject *obj, u16 id, u32 mod_reason, bool no_emerge)
{
	...
	if (!block) {
		if (!no_emerge)
			errorstream << "ServerEnv: Failed to emerge block " << PP(
					obj->m_static_block)
