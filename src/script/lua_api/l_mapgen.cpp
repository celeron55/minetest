...
bool read_deco_schematic(lua_State *L, SchematicManager *schemmgr,
		DecoSchematic *deco);
		...
		Schematic *schem, std::vector<std::string> *names)
{
	...
	for (lua_pushnil(L); lua_next(L, -2); i++, lua_pop(L, 1)) {
		...
		std::unordered_map<std::string, content_t>::iterator it = name_id_map.find(
				name);
...
int ModApiMapgen::l_set_mapgen_params(lua_State *L)
{
	...
	if (lua_isnumber(L, -1))
		settingsmgr->setMapSetting("water_level", readParam<std::string>(L, -1),
			true);
...
bool read_deco_simple(lua_State *L, DecoSimple *deco)
{
	...
	if (param2 < 0 || param2 > 255 || param2_max < 0 || param2_max > 255) {
		errorstream <<
				"register_decoration: param2 or param2_max out of bounds (0-255)"
				<< std::endl;
		return false;
	}
...
int ModApiMapgen::l_read_schematic(lua_State *L)
{
	...
	//// Read options
	std::string write_yslice = getstringfield_default(L, 2, "write_yslice_prob",
			"all");
