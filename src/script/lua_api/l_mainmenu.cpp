		...
		for (unsigned int i = 0; i < number_of_files; i++) {
			...
			if (!files_in_zip->isDirectory(i)) {
				...
				while (total_read < toread->getSize()) {
					...
					if ((bytes_read == 0) ||
							(fwrite(read_buffer, 1, bytes_read,
							targetfile) != bytes_read))
					{
						fclose(targetfile);
						fs->removeFileArchive(fs->getFileArchiveCount()-1);
						lua_pushboolean(L, false);
...
bool ModApiMainMenu::mayModifyPath(const std::string &path)
{
	...
	if (fs::PathStartsWith(path,
			fs::RemoveRelativePathComponents(porting::path_user + DIR_DELIM "games")))
		return true;

	if (fs::PathStartsWith(path,
			fs::RemoveRelativePathComponents(porting::path_user + DIR_DELIM "mods")))
		return true;
	...
	if (fs::PathStartsWith(path,
			fs::RemoveRelativePathComponents(porting::path_user + DIR_DELIM "worlds")))
		return true;
