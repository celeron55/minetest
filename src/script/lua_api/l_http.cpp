...
void ModApiHttp::read_http_fetch_request(lua_State *L, HTTPFetchRequest &req)
{
	...
	if (lua_istable(L, 2)) {
		...
		while (lua_next(L, 2) != 0) {
			req.post_fields[readParam<std::string>(L, -2)] = readParam<std::string>(
					L, -1);
			lua_pop(L, 1);
		}
...
int ModApiHttp::l_request_http_api(lua_State *L)
{
	...
	http_mods.erase(std::remove(http_mods.begin(),
			http_mods.end(), ' '), http_mods.end());
	...
	mod_list_http.insert(mod_list_http.end(),
			mod_list_trusted.begin(), mod_list_trusted.end());
	if (std::find(mod_list_http.begin(), mod_list_http.end(),
