...
struct AutosendCycle
{
	...
	bool server_side_occlusion_culling_setting;
	s16 block_send_optimize_distance_setting;
			...
			*/
			...
			if(block != NULL)
			{
				...
				if (mapblock.d >= block_send_optimize_distance_setting) {
					...
					// not be worth transferring to the client, while they
					// actually are worth transferring.
				...
				if (server_side_occlusion_culling_setting) {
					const v3s16 camera_p_nodes = floatToInt(camera_p, BS);
					if (env->getMap().isBlockOccluded(block, camera_p_nodes))
						continue;
				}
			}
