		...
		std::map<v3s16, MapBlock*> *modified_blocks)
{
	...
	for (blockpos.Y = minblock.Y; blockpos.Y <= maxblock.Y; blockpos.Y++)
		...
		for (relpos.Z = 0; relpos.Z < MAP_BLOCKSIZE; relpos.Z++)
			...
			MapNode node = block->getNodeNoCheck(relpos.X, relpos.Y,
					relpos.Z,
					&is_valid);
			...
			for (size_t b = 0; b < 2; b++) {
				...
				f.light_source;
				if (light > 1)
					relight[b].push(light, relpos, blockpos, block,
							6);
			} // end of banks
		}// end of nodes
	}// end of blocks
	...
	for (blockpos.Y = minblock.Y; blockpos.Y <= maxblock.Y; blockpos.Y++)
		...
		for (const VoxelArea &a : block_pad) {
			...
			for (relpos.X = a.MinEdge.X; relpos.X <= a.MaxEdge.X; relpos.X++)
				for (relpos.Z = a.MinEdge.Z; relpos.Z <= a.MaxEdge.Z;
			relpos.Z++)
			...
			relpos.Y++) {
				...
				MapNode newnode = vm->getNodeNoExNoEmerge(
						relpos + offset);
				const ContentFeatures &newf = oldnode ==
						newnode ? oldf :
				ndef->get(newnode);
...
VoxelLineIterator::VoxelLineIterator(const v3f &start_position,
