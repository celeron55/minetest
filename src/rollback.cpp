...
int RollbackManager::getActorId(const std::string &name)
{
	...
	SQLOK(sqlite3_bind_text(stmt_knownActor_insert, 1, name.c_str(), name.size(),
			NULL));
...
int RollbackManager::getNodeId(const std::string &name)
{
	...
	SQLOK(sqlite3_bind_text(stmt_knownNode_insert, 1, name.c_str(),
			name.size(), NULL));
...
bool RollbackManager::initDatabase()
{
	...
	while (sqlite3_step(stmt_knownActor_select) == SQLITE_ROW) {
		registerNewActor(
				...
				reinterpret_cast<const char*>(sqlite3_column_text(stmt_knownActor_select,
				1))
		);
	}
	...
	while (sqlite3_step(stmt_knownNode_select) == SQLITE_ROW) {
		registerNewNode(
				...
				reinterpret_cast<const char*>(sqlite3_column_text(stmt_knownNode_select,
				1))
		);
	}
...
bool RollbackManager::registerRow(const ActionRow &row)
{
	...
	if (row.type == RollbackAction::TYPE_MODIFY_INVENTORY_STACK) {
		...
		SQLOK(sqlite3_bind_text(stmt_do, 4, row.list.c_str(), row.list.size(),
				NULL));
	...
	if (row.type == RollbackAction::TYPE_SET_NODE) {
		...
		SQLOK(sqlite3_bind_text(stmt_do, 16, row.oldMeta.c_str(), row.oldMeta.size(),
				NULL));
		...
		SQLOK(sqlite3_bind_text(stmt_do, 20, row.newMeta.c_str(), row.newMeta.size(),
				NULL));
...
const std::list<RollbackAction> RollbackManager::getActionsSince_range(
		time_t start_time, v3s16 p, int range, int limit)
{
	return rollbackActionsFromActionRows(getRowsSince_range(start_time, p, range,
			limit));
}
...
void RollbackManager::migrate(const std::string &file_path)
{
	...
	do {
		...
		if (time(0) - t >= 1) {
			...
			std::cout
					<< " Done: " <<
					static_cast<int>((static_cast<float>(fh.tellg()) /
