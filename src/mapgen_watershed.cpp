...
MapgenWatershedParams::MapgenWatershedParams()
{
	...
	np_ridge        = NoiseParams(0.5,  0.1,  v3f(6144, 6144, 6144), 27,      3, 0.6,  2.0);
...
{
	settings->setFlagStr("mgwatershed_spflags", spflags, flagdesc_mapgen_watershed, (u32)-1);
	...
	for (s16 x = node_min.X; x <= node_max.X; x++, index++) {
		...
		for (s16 y = node_max.Y; y >= node_min.Y; y--) {
			...
			if (c == c_stone) {
				...
				nplaced = (u16)-1; // Disable because riverbed sand is present
				air_above = false;
				water_above = true;
