		...
		*/
		...
		for (u16 i = 0; i < 6; i++) {
			...
			case LIQUID_SOURCE:
				...
				if (m_nodedef->getId(cfnb.liquid_alternative_flowing) !=
						liquid_kind) {
					neutrals[num_neutrals++] = nb;
				} else {
		...
		if ((num_sources >= 2 && m_nodedef->get(liquid_kind).liquid_renewable) ||
			...
			new_node_content = m_nodedef->getId(m_nodedef->get(
					liquid_kind).liquid_alternative_source);
...
std::string ServerMap::getSectorDir(v2s16 pos, int layout)
{
	...
	case 2:
		porting::mt_snprintf(cc, sizeof(cc),
				(std::string("%.3x") + DIR_DELIM + "%.3x").c_str(),
...
v2s16 ServerMap::getSectorPos(const std::string &dirname)
{
	...
	else if (component.size() == 3)
	{
		...
		r = sscanf(component.c_str(), (std::string(
				"%3x") + DIR_DELIM + "%3x").c_str(), &x, &y);
	...
	for (s32 x = p_min.X; x<=p_max.X; x++)
	{
		...
		if (block_data_inexistent)
		{
			...
			if (load_if_inexistent && !blockpos_over_max_limit(p)) {
				...
				*/
				VoxelArea a(p *MAP_BLOCKSIZE, (p+1)*MAP_BLOCKSIZE-v3s16(1, 1,
						1));
				...
				for (s32 y = a.MinEdge.Y; y<=a.MaxEdge.Y; y++)
				{
					...
					memset(&m_flags[i], VOXELFLAG_NO_DATA,
							MAP_BLOCKSIZE);
				}
			}
		}
