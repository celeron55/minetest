...
#include "network/socket.h"
	...
	#error \
	"Irrlicht 1.8.2 is known to be broken - please update Irrlicht to version >= 1.8.3"
#endif
...
static void set_allowed_options(OptionList *allowed_options)
{
	...
	allowed_options->insert(std::make_pair("migrate-players",
			...
			_(
			"Migrate from current players backend to another (Only works when using minetestserver or with --server)"))));
	allowed_options->insert(std::make_pair("migrate-auth",
			ValueSpec(VALUETYPE_STRING,
			_(
			"Migrate from current auth backend to another (Only works when using minetestserver or with --server)"))));
	allowed_options->insert(std::make_pair("terminal", ValueSpec(VALUETYPE_FLAG,
			_(
			"Feature an interactive terminal (Only works when using minetestserver or with --server)"))));
#ifndef SERVER
		...
		const Settings &cmd_args)
{
	...
	++it) {
		...
		if (!data.empty()) {
			...
			errorstream << "Failed to load block " << PP(*it) << ", skipping it." <<
					std::endl;
		}
