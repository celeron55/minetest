...
struct CraftReplacements
{
	...
	CraftReplacements(const std::vector<std::pair<std::string,
				std::string> > &pairs_) :
	pairs(pairs_)
	{}
	std::string dump() const;
...
public:
	...
	*/
	...
	virtual CraftOutput getOutput(const CraftInput &input,
			IGameDef *gamedef) const = 0;
	// the inverse of the above
	virtual CraftInput getInput(const CraftOutput &output,
			IGameDef *gamedef) const = 0;
...
public:
	...
	virtual bool clearCraftsByOutput(const CraftOutput &output,
			IGameDef *gamedef) = 0;
